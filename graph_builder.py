...
def build_sub_graph_join(g: Graph, g2: Graph) -> Graph:
    ...
    file_path_csv2 = remap.getPath(g2)
    o_term_map2, o_term_map_type2 = remap.getObject(g2)
    ...
    file_path_csv = remap.getPath(g)

    s_term_map, s_term_map_type = remap.getSubject(g)
    s_term_type = get_term_type_of_graph(g, "s")

    g_term_map, g_term_map_type = remap.getGraph(g)
    g_term_type = "iri"

    p_term_map, p_term_map_type = remap.getPredicate(g)

    o_term_map, o_term_map_type = remap.getObject(g)
